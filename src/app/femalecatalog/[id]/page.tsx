...
const ProductDetail = () => {
  ...
  const [product, setProduct] = useState<FemaleListProps | null>(null);
  ...
  const fetchFemaleList = async () => {
    try {
      ...
      if (result?.femalesCatalogs && Array.isArray(result.femalesCatalogs)) {
        const selectedProduct = result.femalesCatalogs.find(
          (item: FemaleListProps) =>
            item.id === id ||
            item.femalesCatalogTitle.replace(/\s+/g, "-").toLowerCase() === id
        );
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 grid grid-cols-1 md:grid-cols-2 gap-10">
      ...
      <div className="relative w-full h-[500px] overflow-hidden group">
        <Image
          src={product.femalesCatalogImage?.url || "/placeholder.jpg"}
          alt={product.femalesCatalogTitle}
          width={500}
          height={700}
          unoptimized={true}
      ...
      <div>
        <h1 className="text-3xl font-semibold text-gray-600">
          {product.femalesCatalogTitle || "Product Name Not Available"}
        </h1>

        {/* Star Ratings & Reviews */}
        <div className="flex items-center mt-2">
          {Array(
            product.femalesRatingNumber
              ? parseInt(product.femalesRatingNumber)
              : 0
          )
            .fill("")
            .map((_, index) => (
          ...
          <p className="ml-2 text-gray-500">
            {product.femalesRatingNumber}.0 Reviews
          </p>
        </div>
        ...
        <div className="mt-2 flex items-center justify-between">
          ...
          <div>
            <p className="text-gray-500 line-through text-sm">
              RS{" "}
              {product.femalesDiscountPrice
                ? `${product.femalesDiscountPrice}.00`
                : "N/A"}
            </p>
            <p className="text-2xl font-semibold text-gray-700">
              RS{" "}
              {product.femalesCatalogPrice
                ? `${product.femalesCatalogPrice}.00`
                : "Price not available"}
            </p>
          </div>
        ...
        <p className="text-sm text-gray-500 mt-2">
          ...
          {product.femalesCatalogPrice
            ? (parseFloat(product.femalesCatalogPrice) / 4).toFixed(2)
            : "N/A"}{" "}
          with Afterpay
        </p>

        <ul className=" text-gray-700 mt-4">
          {product.femalesCatalogDescription ||
        ...
        <form ref={formRef} onSubmit={onSubmit} className="space-y-4 mt-4">
          ...
          <input
            type="hidden"
            name="product Title"
            value={product.femalesCatalogTitle}
          />
          <input
            type="hidden"
            name="product Price"
            value={product.femalesCatalogPrice}
          />
          <input
            type="hidden"
            name="product Image"
            value={product.femalesCatalogImage?.url || ""}
          />
