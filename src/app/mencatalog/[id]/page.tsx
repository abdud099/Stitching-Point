...
const ProductDetail = () => {
  ...
  const fetchMenList = async () => {
    try {
      ...
      if (result?.mensCatalogs && Array.isArray(result.mensCatalogs)) {
        const selectedProduct = result.mensCatalogs.find(
          (item: MenListProps) =>
            item.id === id ||
            item.mensCatalogTitle.replace(/\s+/g, "-").toLowerCase() === id
        );
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 grid grid-cols-1 md:grid-cols-2 gap-10">
      ...
      <div className="relative w-full h-[500px] overflow-hidden group">
        <Image
          src={product.mensCatalogImage?.url || "/placeholder.jpg"}
          alt={product.mensCatalogTitle}
          width={500}
          height={700}
          unoptimized={true}
